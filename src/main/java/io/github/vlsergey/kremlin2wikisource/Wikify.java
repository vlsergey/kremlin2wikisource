...
class Wikify {
    ...
    static String wikify(String content) {
        ...
        content = content.replaceAll(
                " (завод|комбинат|отряда|поезда|предприятия|станции|трест|управление|фабрики|школы) N (\\d+)([ ,»])",
                " $1 {{nobr|№ $2}}$3");
        content = content.replaceAll(" \\- ", " — ");
        content = content.replaceAll("==\n\n([А-Яа-я])", "==\n$1");
        return content;
    ...
    static String wikifyQuotes(String content) {
        ...
        emphasizeTokens = emphasizeTokens.replace(".☆", "☆.☆");
        emphasizeTokens = emphasizeTokens.replaceAll("\\.$", "☆.");
        content = Arrays.asList(StringUtils.splitPreserveAllTokens(emphasizeTokens, "☆"))
                .stream()
                .map(Wikify::wikifyQuotesImpl)
