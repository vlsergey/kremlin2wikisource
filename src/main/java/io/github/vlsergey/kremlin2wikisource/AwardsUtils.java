...
class AwardsUtils {
	...
	static String wikilink(String content) {
		...
		content = wikilinkImpl(content, "МЕДАЛЬЮ «ЗА ОТЛИЧИЕ В ОХРАНЕ ОБЩЕСТВЕННОГО ПОРЯДКА»",
				"Медаль «За отличие в охране общественного порядка»");
		content = wikilinkImpl(content, "МЕДАЛЬЮ «ЗА СПАСЕНИЕ ПОГИБАВШИХ»", "Медаль «За спасение погибавших»");

		content = replaceAll(content, "\\n=== [«\"]НАРОДНЫЙ (.*) РОССИЙСКОЙ ФЕДЕРАЦИИ[»\"] ===\n", true,
				(matcher, g1) -> "\n=== [[:w:ru:Народный " + g1.toLowerCase() + " Российской Федерации|«НАРОДНЫЙ " + g1
						+ " {{nobr|РОССИЙСКОЙ ФЕДЕРАЦИИ}}»]] ===\n");
